...
<style>
    ...
    /* 2. Ziwei Grid - Wenmo Tianji Style */
    .ziwei-grid {
        display: grid;
    ...
    .star-adj {
        color: #6d4c41; /* Adjective Star Brown */
        font-size: 1.1em; 
        margin-right: 2px;
        display: inline-block;
    }
...
# ==========================================
# 1. Calculation & Helper Functions
# ==========================================

# Inline Solar Time Calculation
...
def get_chinese_hour(hour, minute):
    """Maps hour/minute to Chinese zodiac hour."""
    chinese_hours = [
        ("子时", 11), ("丑时", 1), ("寅时", 2), ("卯时", 3),
        ("辰时", 4), ("巳时", 5), ("午时", 6), ("未时", 7),
        ("申时", 8), ("酉时", 9), ("戌时", 10), ("亥时", 11),
    ]
    total_minutes = hour * 60 + minute
    if 23*60 <= total_minutes or total_minutes < 1*60: return chinese_hours[0]
    elif 1*60 <= total_minutes < 3*60: return chinese_hours[1]
    elif 3*60 <= total_minutes < 5*60: return chinese_hours[2]
    elif 5*60 <= total_minutes < 7*60: return chinese_hours[3]
    elif 7*60 <= total_minutes < 9*60: return chinese_hours[4]
    elif 9*60 <= total_minutes < 11*60: return chinese_hours[5]
    elif 11*60 <= total_minutes < 13*60: return chinese_hours[6]
    elif 13*60 <= total_minutes < 15*60: return chinese_hours[7]
    elif 15*60 <= total_minutes < 17*60: return chinese_hours[8]
    elif 17*60 <= total_minutes < 19*60: return chinese_hours[9]
    elif 19*60 <= total_minutes < 21*60: return chinese_hours[10]
    else: return chinese_hours[11]
...
def solar_time_to_chinese_hour(year, month, day, hour, minute, longitude=120.033):
    ...
    result = {
        ...
        "time_difference": f"{true_hour - hour:+.1f}h {true_minute - minute:+.1f}m"
    }
    return result
...
# ==========================================
# 2. Data Fetching & Prompt Generation
# ==========================================

def get_ziwei_data(birthday, hour_index, gender, target_year, is_lunar=False, is_leap=False):
    """
    Fetches Ziwei data from API or falls back to mock data.
    """
    try:
        payload = {
            "birthday": birthday,
        ...
        try:
            ...
            st.warning("⚠️ Local API unavailable. Using mock data for demonstration.")
...
def parse_ziwei_to_prompt(full_data):
    """Generates initial system prompt based on static Natal Chart."""
    pan = full_data['astrolabe']
    
    # Solar Time Info from Session
    solar_result = st.session_state.get('solar_result', {})
    clock_time = solar_result.get('clock_time', f"{pan['solarDate']} {pan.get('timeRange', '').split('~')[0]}")
    true_solar_time = solar_result.get('true_solar_time', f"{pan['solarDate']} {pan.get('timeRange', '').split('~')[0]}")
    ...
    农历时间：{pan['lunarDate']} {chinese_hour}
    节气四柱：{pan['chineseDate']}
    身主：{pan['body']} | 命主：{pan['soul']} | 子年斗君：寅 | 身宫：{pan['earthlyBranchOfBodyPalace']}
    """
    ...
    for p in pan['palaces']:
        ...
        palace_text += f"  ├大限 : {decadal_range[0]}~{decadal_range[1]}岁\n"
        palace_text += f"  └流年 : {', '.join(map(str, ages[:5]))}...\n\n"
...
def render_html_grid(full_data):
    ...
    def make_cell(dizhi):
        ...
        if major_stars:
            stars_html += f'<div class="star-section">{"".join(major_stars)}</div>'
        ...
        if minor_stars:
            stars_html += f'<div class="star-section">{"".join(minor_stars)}</div>'
        ...
        if adj_stars:
            stars_html += f'<div class="star-section">{"".join(adj_stars)}</div>'
        ...
        stars_html += f'流年: {",".join(map(str, yearly_ages))}<br>'
        stars_html += f'小限: {",".join(map(str, minor_ages))}'
        stars_html += f'</div></div>'
...
with st.sidebar:
    ...
    # Page Navigation
    page = st.radio("选择页面", ["命盘显示", "AI 命理咨询师"], index=0)
    st.markdown("---")
    ...
    if cal_type == "公历":
        ...
        with col_h: sel_hour = st.selectbox("时", list(range(24)), index=10)
        with col_m: sel_minute = st.selectbox("分", list(range(0, 60, 5)), index=10)
...
if 'birth_date_str' in st.session_state and 'ziwei_data' in st.session_state:
    ...
    if page == "命盘显示":
        ...
        # Safety check for decades
        if decades and selected_decade_idx < len(decades):
        ...
        if prompt := st.chat_input("输入你的问题..."):
            ...
            if "ziwei_data" in st.session_state and st.session_state["ziwei_data"]:
                ...
                system_prompt = "请先生成命盘数据。"
            
            dynamic_messages = [{"role": "system", "content": system_prompt}]
